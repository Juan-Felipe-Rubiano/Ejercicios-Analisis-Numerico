...
Description: Método de las sumas superiores para calcular areas bajo la curva de una función dada. El método consiste en
 dividir el intervalo de integración en n subintervalos, calcular el valor de la función en el extremo derecho de cada
 subintervalo, y luego multiplicar ese valor por el ancho del subintervalo para obtener la suma superior.
"""
...
def main():
    ...
    n_in = None
    print(
        "Para calcular el área bajo la curva de una función utilizando el método de las sumas superiores, digite los siguientes parámetros:\n")
    while not f_in:
        f_in = input(
    ...
    while n_in is None:
        try:
            n_in = int(input(
                "Digite el número de subintervalos para aplicar el método de las sumas superiores (10 por defecto):\n") or 10)
            if n_in <= 0:
    ...
    try:
        area = sumas_superiores(f_in, a_in, b_in, n_in)
        print("El área bajo la curva de la función es:", area)
    except ValueError as e:
        print("Error:", e)


def sumas_superiores(f_in, a, b, n):
    x = sp.symbols('x')
    try:
